...
| 指标 | 权重 | 规则 |
| --- | ---: | --- |
| 折价 | 60 | 6.5 折及以下满分，10 折及以上为 0 |
| 流动性：买卖价差 | 24 | 2% 及以下满分，20% 及以上为 0 |
| 流动性：挂牌深度 | 16 | 在售数量按对数归一化，1000 件封顶 |
...
同时取得两侧数量时，挂牌深度按 BUFF 75% + Steam 25% 合成；Steam 数量缺失时只使用 BUFF 数量，不会按零分处理。鼠标悬浮或键盘聚焦评分可查看该商品的实际得分明细。
...
Steam Community 市场搜索用于获取最低在售价和当前在售数量。该网页接口可能限流或变更；启用 `steam_buff_fallback` 后，直查失败时会使用 BUFF 同步的 Steam 参考价，但此时没有 Steam 实时在售数量。
