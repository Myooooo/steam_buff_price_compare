...
前端采用深色游戏主题的数据控制台，支持最新/缓存数据标记、关键词、枪械、类别、磨损、价格、来源、盈利条件筛选和多维排序。点击商品行会直接打开对应的 BUFF 商品页。
...
- 深度扫描的阶段、页码和定价进度持久化到 SQLite，支持暂停和重启续传。
- 深度任务运行时，立即扫描会在安全检查点优先执行，结束后自动恢复深度任务。
- SQLite 保存商品快照、价格历史、扫描记录、深度索引和检查点。
...
1. `indexing`：按 BUFF 市场价格升序遍历所有分页，将完整商品元数据写入 `deep_scan_index`。
...
`enabled` 只控制定时调度。手动启动、暂停和续传不受该开关限制。
...
启动时会自动检查并迁移旧版 `items` 表，补充 BUFF 商品 ID、图片、分类和扫描批次字段。迁移不会清空已有商品或历史记录。重要数据仍建议自行备份 `data/prices.db`。
...
| `POST` | `/api/scan/deep/pause` | 暂停当前或等待恢复的深度任务 |
...
item_type         类别
exterior          磨损
min_price         BUFF 最低价下限
max_price         BUFF 最低价上限
...
测试覆盖手续费与价格解析、BUFF 元数据标准化、配置持久化、扫码登录、全分页扫描、深度索引检查点、关键词优先级、筛选排序、新鲜度标签、数据库迁移、错误和取消状态。
...
旧版本数据没有保存 BUFF 分类元数据。完成一次新的关键词扫描后，对应商品会补齐图片、枪械、类别和磨损字段，筛选项会自动出现。
